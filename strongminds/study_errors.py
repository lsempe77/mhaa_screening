...
import strongminds.fewshot_screen as F
...
gt_raw = json.loads((ROOT / "strongminds/data/gt_510.json").read_text(encoding="utf-8"))
gt_bin = {k: ("INCLUDE" if "INCLUDE" in v.upper() else "EXCLUDE") for k, v in gt_raw.items()}
adj = {a["record_id"]: a for a in (json.loads(l) for l in lines("strongminds/gt_adjudication.jsonl"))}
res = json.loads((HERE / "fewshot_results_k1.json").read_text(encoding="utf-8"))
