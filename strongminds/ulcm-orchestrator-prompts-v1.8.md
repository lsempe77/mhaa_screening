...
BIOLOGICAL-MECHANISM EXCLUSION (review-team decision, 2026-07-21): RQ1 determinants are limited to risk factors that are modifiable / social / psychological / economic / contextual and assessable in community or lay-delivered settings (the ULCM framing). FAIL biological / neurobiological / genetic mechanism studies that merely correlate with depression: neuroimaging correlates of MDD (brain connectivity, inter-brain synchrony, face-processing networks), biomarker studies (salivary/circulating markers of depression), genetic-correlation studies (5-HTTLPR / OXTR / HPA-axis polymorphisms, Mendelian randomization drug-target screens). These are basic-science mechanism research ("biomarkers keep a hospital setup"), not community-actionable determinants. EXCEPTION: a biological marker study PASSES only when it validates a screening or measurement instrument usable in community/lay-delivered settings (e.g. validating a salivary marker as a lay-administered depression screen) — i.e. it functions as an RQ18 measurement study, not a mechanism study.
...
- DETERMINANTS (RQ1) — biological / neurobiological / genetic mechanism studies (neuroimaging correlates, biomarkers, genetic polymorphisms, Mendelian randomization) are OUT — only modifiable/social/psychological/economic risk factors and prevalence are determinants. EXCEPTION: a biological marker study validating a community-usable depression screen counts as RQ18 measurement, not a mechanism study.
...
- OUTCOME (intervention routes) — the test is whether the INTERVENTION TARGETS DEPRESSION, not whether depression is the primary outcome. A depression-targeting intervention applied to a comorbid population is IN-scope even if depression is a co-primary/secondary outcome.
