...
Reads the Zotero references CSV (projects/girl_effect/full_text/references_*.csv), extracts text from each PDF
in projects/girl_effect/full_text/pdfs/ via PyMuPDF (fitz), and writes the same records.jsonl format that
k5_runner.py consumes — with the full PDF text in the `abstract` field (so the existing
prompt/runner pipeline works unchanged).

Outputs:
    projects/girl_effect/full_text/data/records_<n>.jsonl   one record per PDF-bearing row, fields:
                                      record_id, year, title, abstract (full text),
                                      n_pages, n_chars, pdf_file
    projects/girl_effect/full_text/data/missing_pdf.jsonl   the rows with has_pdf=False (for separate follow-up)
...
def main():
    ...
    p.add_argument("--out-dir", default="projects/girl_effect/full_text/data", help="Output directory.")
    ...
    print(f"\nNext step:")
    print(f"  python pipeline/k5_runner.py \\")
    print(f"      --prompt projects/girl_effect/prompts/prompts-screening-mhaa-fulltext-v1.md \\")
    print(f"      --records {records_path} \\")
    print(f"      --out projects/girl_effect/full_text/output/results_fts_glm_{n}.jsonl \\")
