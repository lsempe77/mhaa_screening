...
import fewshot_screen as F   # reuse RUBRIC, MODELS, screen_one, fmt_exemplars, build_index
...
def load():
    ...
    ris = {}
    for l in lines("projects/strongminds/data/ris_records.jsonl"):
        r = json.loads(l)
        if r["record_id"]:
...
def main():
    ...
    if args.pilot:
        ...
        (ROOT / "projects/strongminds/artifacts/ris_pilot_scores.json").write_text(json.dumps(out, ensure_ascii=False), encoding="utf-8")
        print(f"\nWrote {ROOT/'projects/strongminds/artifacts/ris_pilot_scores.json'}")
        ...
        with (ROOT / "projects/strongminds/artifacts/ris_full_scores.jsonl").open("w", encoding="utf-8") as fh:
            for rid in targets:
                r = ris[rid]
                fh.write(json.dumps({"record_id": rid, "score": scores[rid],
                                     "title": r["title"], "year": r["year"]}, ensure_ascii=False) + "\n")
        print(f"Wrote {ROOT/'projects/strongminds/artifacts/ris_full_scores.jsonl'}")
    else:
