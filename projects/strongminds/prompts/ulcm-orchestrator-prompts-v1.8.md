...
- INTERVENTION (intervention routes) — the ULCM scope is BRIEF, STRUCTURED, GROUP-DELIVERED, LAY/PEER-FACILITATED psychological interventions. Psychodynamic psychotherapy (STPP, ISTDP, dynamic psychotherapy), couple/marital therapy, and psychoanalysis are OUT — they are specialist-delivered individual/dyadic modalities outside the ULCM delivery model. Do not PASS an intervention solely because it is "structured psychological" — it must also fit the brief/group/lay-deliverable ULCM model.
- OUTCOME (intervention routes) — the test is whether the INTERVENTION TARGETS DEPRESSION, not whether depression is the primary outcome. A depression-targeting ULCM-eligible intervention applied to a comorbid population is IN-scope even if depression is a co-primary/secondary outcome. FAIL only when the intervention targets a non-depression condition and depression is merely measured.
